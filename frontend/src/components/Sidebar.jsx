...
import { 
  ...
  Cpu, Info, Sliders, ChevronRight, X
} from 'lucide-react';
...
export default function Sidebar({ 
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex w-64 bg-[#FFE2E2] border-r border-[#F5CBCB] flex-col h-screen sticky top-0 select-none z-40">
        {/* Brand Header */}
      ...
      {/* Mobile Drawer Navigation (visible on < lg when open) */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden flex">
          ...
          {/* Slide-out Panel */}
          <aside className="relative flex-1 max-w-xs w-full bg-[#FFE2E2] flex flex-col h-full z-10 shadow-2xl animate-fade-in select-none">
            <div className="p-4 border-b border-[#F5CBCB] flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-[#FFFFFF] border border-[#F5CBCB] p-1 shadow-sm flex items-center justify-center">
                  <img src="/logo.png" alt="Churn Analytics Logo" className="w-full h-full object-contain" />
                </div>
              ...
              <button
                onClick={() => setIsMobileMenuOpen(false)}
                className="p-2 rounded-xl text-[#7A5C77] hover:text-[#2D1E2F] hover:bg-[#F5CBCB] transition-colors"
